    ...
    <div class="bg-white shadow-lg rounded-xl border border-gray-100 overflow-hidden">
        <div class="p-6 sm:p-8">
            <div class="flex items-center justify-between mb-6">
                <h3 class="text-lg font-medium text-gray-900">Form Edit Absensi</h3>
            ...
            <form action="{{ route('admin.absensi.update', $absensi->id) }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-2">
                        ...
                        <div class="mt-1">
                            <select name="user_id" id="user_id" required
                                ...
                                @foreach($mahasiswa as $m)
                                    <option value="{{ $m->id }}" {{ old('user_id', $absensi->user_id) == $m->id ? 'selected' : '' }}>{{ $m->name }} ({{ $m->nim }})</option>
                                @endforeach
                            </select>
                        </div>
                        @error('user_id')
                    ...
                    <div>
                        ...
                        <div class="mt-1">
                            <input type="time" name="jam_masuk" id="jam_masuk" value="{{ old('jam_masuk', $absensi->jam_masuk ? \Carbon\Carbon::parse($absensi->jam_masuk)->format('H:i') : '') }}"
                                class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-lg transition duration-200">
                        </div>
                        @error('jam_masuk')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <div class="mt-1">
                            <input type="time" name="jam_keluar" id="jam_keluar" value="{{ old('jam_keluar', $absensi->jam_keluar ? \Carbon\Carbon::parse($absensi->jam_keluar)->format('H:i') : '') }}"
                                class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-lg transition duration-200">
                        </div>
                        @error('jam_keluar')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <div class="mt-1">
                            <textarea name="keterangan" id="keterangan" rows="3"
                                class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-lg transition duration-200" placeholder="Tambahkan keterangan jika perlu">{{ old('keterangan', $absensi->keterangan) }}</textarea>
                        </div>
                        @error('keterangan')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror

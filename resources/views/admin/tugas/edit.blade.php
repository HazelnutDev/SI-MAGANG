...
@section('header', 'Edit Data Tugas')
    ...
    <div class="bg-white shadow-lg rounded-xl border border-gray-100 overflow-hidden">
        <div class="p-6 sm:p-8">
            <div class="flex items-center justify-between mb-6">
                <h3 class="text-lg font-medium text-gray-900">Form Edit Tugas</h3>
            ...
            <form action="{{ route('admin.tugas.update', $tugas->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-2">
                    <div class="sm:col-span-2">
                        ...
                        <div class="mt-1">
                            <input type="text" name="judul_tugas" id="judul_tugas" value="{{ old('judul_tugas', $tugas->judul_tugas) }}" required
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <div class="mt-1">
                            <textarea name="deskripsi" id="deskripsi" rows="4" required
                                class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-lg transition duration-200">{{ old('deskripsi', $tugas->deskripsi) }}</textarea>
                        </div>
                        @error('deskripsi')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <div class="mt-1">
                            <input type="date" name="tanggal_mulai" id="tanggal_mulai" value="{{ old('tanggal_mulai', optional($tugas->tanggal_mulai)->format('Y-m-d')) }}" required
                                class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-lg transition duration-200">
                        </div>
                        @error('tanggal_mulai')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <div class="mt-1">
                            <input type="date" name="tanggal_selesai" id="tanggal_selesai" value="{{ old('tanggal_selesai', optional($tugas->tanggal_selesai)->format('Y-m-d')) }}" required
                                class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-lg transition duration-200">
                        </div>
                    ...
                    <div>
                        ...
                        <div class="mt-1">
                            <select name="status" id="status" required
                                class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-lg transition duration-200">
                                <option value="pending" {{ old('status', $tugas->status) == 'pending' ? 'selected' : '' }}>Pending</option>
                                <option value="active" {{ old('status', $tugas->status) == 'active' ? 'selected' : '' }}>Active</option>
                                <option value="completed" {{ old('status', $tugas->status) == 'completed' ? 'selected' : '' }}>Completed</option>
                            </select>
                        </div>
                        @error('status')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        ...
                        <div class="border border-gray-200 rounded-lg overflow-hidden">
                            ...
                            <div class="max-h-60 overflow-y-auto divide-y divide-gray-200 bg-white">
                                @foreach($mahasiswa as $index => $m)
                                    @php
                                        $assigned = $tugas->tugasMahasiswa->where('user_id', $m->id)->first();
                                        $isChecked = $assigned ? true : false;
                                        $category = $assigned ? $assigned->kategori : 'photographer';
                                    @endphp
                                <div class="px-4 py-3 grid grid-cols-12 gap-4 items-center hover:bg-gray-50 transition duration-150">
                                    <div class="col-span-1">
                                        <input type="checkbox" name="mahasiswa[{{ $index }}][user_id]" value="{{ $m->id }}" {{ $isChecked ? 'checked' : '' }}
                                    ...
                                    <div class="col-span-5">
                                        <select name="mahasiswa[{{ $index }}][kategori]" class="block w-full text-xs border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                            <option value="photographer" {{ $category == 'photographer' ? 'selected' : '' }}>Photographer</option>
                                            <option value="videographer" {{ $category == 'videographer' ? 'selected' : '' }}>Videographer</option>
                                            <option value="pressrelease" {{ $category == 'pressrelease' ? 'selected' : '' }}>Press Release</option>
                                        </select>
                                    </div>
                                </div>
                                @endforeach

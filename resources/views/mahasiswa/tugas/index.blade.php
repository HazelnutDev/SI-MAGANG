...
@section('title', 'Tugas')
@section('header', 'Tugas Kegiatan Harian')

@section('content')
<div class="bg-white shadow-lg rounded-xl border border-gray-100 overflow-hidden mb-6">
    <div class="p-6 border-b border-gray-100 bg-gray-50/50">
        <form action="{{ route('mahasiswa.tugas.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div>
            ...
            <div class="flex items-end">
                <div class="flex space-x-2 w-full">
                    <a href="{{ route('mahasiswa.tugas.index') }}" class="flex-1 px-4 py-2 border border-gray-300 rounded-lg shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition duration-200 text-center">Reset</a>
                    <button type="submit" class="flex-1 px-4 py-2 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition duration-200">Filter</button>
                </div>
            </div>
        </form>
    </div>
...
<div class="bg-white shadow-lg rounded-xl border border-gray-100 overflow-hidden">
    <div class="p-6 border-b border-gray-100 bg-gray-50/50 flex flex-col sm:flex-row justify-between items-center gap-4">
        <div class="flex-1 w-full sm:w-auto">
            <form action="{{ route('mahasiswa.tugas.index') }}" method="GET" class="relative group">
                ...
                @forelse($tugas as $row)
                    <tr class="hover:bg-gray-50 transition duration-150 ease-in-out">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm font-medium text-gray-900">{{ $row->tugas->judul_tugas }}</div>
                            <div class="text-sm text-gray-500 line-clamp-1">{{ \Illuminate\Support\Str::limit($row->tugas->deskripsi, 50) }}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-900">
                                {{ \Carbon\Carbon::parse($row->tugas->tanggal_mulai)->format('d M') }} - {{ \Carbon\Carbon::parse($row->tugas->tanggal_selesai)->format('d M Y') }}
                            </div>
                            <div class="text-xs text-gray-500">
                                {{ \Carbon\Carbon::parse($row->tugas->tanggal_selesai)->diffForHumans() }}
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            @php
                            ...
                            @endphp
                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $taskClasses[$row->tugas->status] ?? 'bg-gray-100 text-gray-800' }}">
                                {{ ucfirst($row->tugas->status) }}
                            </span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-right">
                            <a href="{{ route('mahasiswa.tugas.show', $row->id) }}" class="text-indigo-600 hover:text-indigo-900 bg-indigo-50 hover:bg-indigo-100 p-1.5 rounded-lg transition-colors duration-200">Detail</a>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="3" class="px-6 py-12 text-center">
                            <p class="text-gray-500 text-lg font-medium">Belum ada tugas.</p>
                        </td>
                    </tr>
                @endforelse
            </tbody>
